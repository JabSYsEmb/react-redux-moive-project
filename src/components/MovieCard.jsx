    ...
    return (
        <div className="inline-block text-center group md:w-200 mb-6 sm:max-w-fit">
            <div className="relative text-center">
                <img className="group-hover:blur-sm shadow" src={`https://image.tmdb.org/t/p/w300/${movie.poster_path}`} alt={movie.title} />
                <span className="absolute px-2 left-1 top-1 rounded text-white bg-orange-400 inline-block">{movie.vote_average}</span>
            ...
            <span className="text-xs text-green-100">{new Date(movie.release_date).toLocaleString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
        </div>
    );
}
